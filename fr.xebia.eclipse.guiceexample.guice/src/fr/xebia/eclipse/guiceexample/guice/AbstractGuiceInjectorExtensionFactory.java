package fr.xebia.eclipse.guiceexample.guice;
...
/**
 * This class is inspired by {@link org.ops4j.peaberry.eclipse.GuiceExtensionFactory} and
 * {@link org.eclipse.springframework.util.SpringExtensionFactory}. It provides a convenient way to
 * let Guice create extensions defined in plugin.xml and inject their dependencies. The difference
 * with GuiceExtensionFactory is that instead of searching for the declaration of a
 * {@link com.google.inject.Module} in plugin.xml to create an {@link com.google.inject.Injector},
 * this abstract factory requires to be subclassed in order to specify an injector to be used. That
 * way you can use an already existing injector if you will.
 * <p>
 * This is especially handy when the extensions defined in plugin.xml are not the only entry point
 * to your plugin, and when you want to use the same module defining singletons to inject the
 * members of both your extensions and the other entry point (GuiceExtensionFactory would
 * re-instantiate the singletons for your extensions).
 * </p>
 * <p>
 * To use your concrete factory put it in front of your class name in the extension XML. Or replace
 * your class with the factory and put your class in the {@code id} attribute instead. Because the
 * implementation will be injected based on the bindings you could even replace your class name with
 * one of its interfaces, and that interface will then be used to lookup the correct implementation.
 * </p>
 * <p>
...
public abstract class AbstractGuiceInjectorExtensionFactory implements IExecutableExtension, IExecutableExtensionFactory {
	...
	public void setInitializationData(final IConfigurationElement config, final String name, final Object data) {
		...
		// if there's no (string-based) adapter data then the class name must be under "id"
